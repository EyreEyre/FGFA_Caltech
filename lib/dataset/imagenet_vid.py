...
class ImageNetVID(IMDB):
    ...
    def load_vid_annotation(self, iindex):
        ...
        for ix, obj in enumerate(objs):
            ...
            # caltech
            # add 1 pixel for xmin, make sure xmin < xmax
            if (x1 >= (roi_rec['width'] - 1) and x2 == (roi_rec['width']-1)):
                x1 = x2 - 1
            # end
...
###################################################################################################
    ...
    def do_python_eval_gen(self, gpu_number=None):
        ...
        ############ eval by classes ################
        ap, recall, precision = vid_eval(multifiles, filenames, annopath, imageset_file, self.classes_map, annocache, ovthresh=0.5)
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print('AP for {} = {:.4f}'.format(cls, ap[cls_ind - 1]))
            ...
            info_str += 'Recall for {} = {:.4f}\n'.format(cls, recall[cls_ind - 1])

            print('Precision for {} = {:.4f}'.format(cls, precision[cls_ind - 1]))
            info_str += 'Precision for {} = {:.4f}\n'.format(cls, precision[cls_ind - 1])
